...
class KeyFrame
{
	...
	//cout << "******----- Interesting: mvKeysRight size: " << mvKeysRight.size() << endl;
        ar & NLeft;
        ar & NRight;
...
public:
    ...
    void PostLoad_second(map<long unsigned int, KeyFrame*>& mpKFid, map<long unsigned int, MapPoint*>& mpMPid, map<unsigned int, GeometricCamera*>& mpCamId);
...
public:
    ...
    //nNextId_backup is to record previous loaded map's maximum mnid to avoid confliction
    static long unsigned int nNextId_backup;//default 0
    static long unsigned int nNextId_max;
