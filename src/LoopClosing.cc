...
{struct timeval mergeStart0, mergeStart, mergeEnd;
gettimeofday(&mergeStart0, NULL);
    mbFinished =false;
    ofstream file_out;
    ...
    while(1)
    {
        ...
        if(CheckNewKeyFrames())
        {
            ...
            cout<<"New KeyFrame inserted!!!!\n";
        gettimeofday(&mergeStart, NULL);
	    //cout << "LoopClosing::Run() 01" << endl;
        gettimeofday(&start, NULL);
            ...
            gettimeofday(&end, NULL);
             double check1 = (end.tv_sec * 1000000 + end.tv_usec)  - (start.tv_sec * 1000000 + start.tv_usec );
    file_out << "-------check1  if(mpLastCurrentKF): " << check1<< endl;
	    //cout << "LoopClosing::Run() 02" << endl;
	    //crashed here
        gettimeofday(&start, NULL);
            if(NewDetectCommonRegions())
            {
                gettimeofday(&end, NULL);
             double check2 = (end.tv_sec * 1000000 + end.tv_usec)  - (start.tv_sec * 1000000 + start.tv_usec );
    file_out << "-------check2  if(NewDetectCommonRegions()) " << check2<< endl;
		//cout << "LoopClosing::Run() 03" << endl;
                gettimeofday(&start, NULL);
                if(mbMergeDetected)
                {
                    gettimeofday(&end, NULL);
             double check3 = (end.tv_sec * 1000000 + end.tv_usec)  - (start.tv_sec * 1000000 + start.tv_usec );
    file_out << "-------check3  if(mbMergeDetected) " << check3<< endl;
		    //cout << "LoopClosing::Run() 03.0" << endl;
		    
		    //gettimeofday(&Tracking::start_create_map, NULL);
                    ...
                    {
			//cout << "LoopClosing::Run() 03.1" << endl;
...
//                        cout << "scale w2w1: " << mSold_new.scale() << endl;
			...
			cout << "start merging" << endl;
                        ...
                        if (mpTracker->mSensor==System::IMU_MONOCULAR ||mpTracker->mSensor==System::IMU_STEREO)
                        {
                            cout << "start merging, MergeLocal2()" << endl;
			    MergeLocal2();
			}
                        else{
			    cout << "start merging, MergeLocal()" << endl;
                {
                    //unique_lock<mutex> lockpassed(passedCheckingMutex);
                    MergeLocal();
            ...
            gettimeofday(&end, NULL);
             double check4 = (end.tv_sec * 1000000 + end.tv_usec)  - (start.tv_sec * 1000000 + start.tv_usec );
        file_out << "-------check4 MergeLocal()) " << check4<< endl;

			cout << "end merging" << endl;
			//The followings might crash because Tracking::start_create_map is not initialized?
			/*gettimeofday(&end, NULL);
			double elapsed = ((end.tv_sec - Tracking::start_create_map.tv_sec) * 1000)
        + (end.tv_usec / 1000 - Tracking::start_create_map.tv_usec / 1000);
			std::ofstream myfile;
			myfile.open ("/home/xukan/Desktop/time.txt", std::ofstream::out | std::ofstream::app);
			myfile << "merge local time: " << elapsed << "ms\n";
  			myfile.close();*/
                    }
        ...
        gettimeofday(&start, NULL);
                if(mbLoopDetected)
                {
                    ...
                    if(mpCurrentKF->GetMap()->IsInertial())
                    {
                        ...
                        {
                            if(mpCurrentKF->GetMap()->IsInertial())//monocular won't be here
                            {
                                // If inertial, force only yaw
                                if ((mpTracker->mSensor==System::IMU_MONOCULAR ||mpTracker->mSensor==System::IMU_STEREO) &&
                            ...
                            gettimeofday(&end, NULL);
             double check45 = (end.tv_sec * 1000000 + end.tv_usec)  - (start.tv_sec * 1000000 + start.tv_usec );
    file_out << "-------check4.5 - 1 CorrectLoop() if(mbLoopDetected) " << check45<< endl;
                        }
                        else
                        {
                            cout << "BAD LOOP!!!" << endl;
                    ...
                    else
                    {
                        mvpLoopMapPoints = mvpLoopMPs;
                        {
                            ...
                            gettimeofday(&end, NULL);
             double check45 = (end.tv_sec * 1000000 + end.tv_usec)  - (start.tv_sec * 1000000 + start.tv_usec );
    file_out << "-------check4.5 - 2  CorrectLoop() if(mbLoopDetected) " << check45<< endl;
                            cout<< "correct loop 2 is done" << endl;
                        }
                    }
file_out << "-------wow, check4.5 mbLoopDetected" <<  endl;
                    // Reset all variables
                    mpLoopLastCurrentKF->SetErase();
                    mpLoopMatchedKF->SetErase();
                    mnLoopNumCoincidences = 0;
                ...
                gettimeofday(&end, NULL);
             double check5 = (end.tv_sec * 1000000 + end.tv_usec)  - (start.tv_sec * 1000000 + start.tv_usec );
    file_out << "-------check5 if(mbLoopDetected) " << check5<< endl;

            }
	    //cout << "LoopClosing::Run() 05" << endl;
...
//file_out.open("timeseris.txt", std::ios_base::app);
    ...
    file_out << "This thread's loopclosing entire merging time is: " << mergeTime<< endl;
    file_out<<"Keyframe ID of merge: "<<mpCurrentKF->mnId<<endl;
    file_out<<"mergeEnd time is: " << mergeEnd.tv_sec * 1000000 + mergeEnd.tv_usec <<endl;
    totalKeyframesInsertion++;
            }
	//cout << "LoopClosing::Run() 06" << endl;
        ResetIfRequested();

        if(CheckFinish()){
            
            gettimeofday(&mergeEnd, NULL);
                file_out << "Check finish done, total time is: " ;
                double allThreadsTime = (mergeEnd.tv_sec * 1000000 + mergeEnd.tv_usec)  - (mergeStart0.tv_sec * 1000000 + mergeStart0.tv_usec );
                file_out<< allThreadsTime <<endl;
             cout << "LC: Finish requested" << endl;
            break;
        }

        usleep(5000);
	//cout << "LoopClosing::Run() end" << endl;
    }
...
void LoopClosing::InsertKeyFrame(KeyFrame *pKF)
{
    ...
    //qunique_lock<mutex> lockpassed(passedCheckingMutex);

    std::cout << "pKF->mnId: "<< pKF->mnId << std::endl;
    std::cout << "before insertion mlpLoopKeyFrameQueue.size() =  "<< mlpLoopKeyFrameQueue.size() << std::endl;
    if(pKF->mnId!=0)   //?? Maynot be right
        mlpLoopKeyFrameQueue.push_back(pKF);

    std::cout << "after insertion  mlpLoopKeyFrameQueue.size() =  "<< mlpLoopKeyFrameQueue.size() << std::endl;
    std::cout << "end of lock" << std::endl;   
}
...
bool LoopClosing::CheckNewKeyFrames()
{
    unique_lock<mutex> lock(mMutexLoopQueue);
    unique_lock<mutex> lockpassed(passedCheckingMutex);
    bool res = !mlpLoopKeyFrameQueue.empty();
    passedChecking = res;
    return res;
}
...
{
    //cout << "LoopClosing::NewDetectCommonRegions() start" << endl;
    {
        unique_lock<mutex> lock(mMutexLoopQueue);
    ...
    //cout << "LoopClosing::NewDetectCommonRegions() 01" << endl;
    if(mpLastMap->IsInertial() && !mpLastMap->GetIniertialBA1())
    {
        mpKeyFrameDB->add(mpCurrentKF);
        mpCurrentKF->SetErase();
	//cout << "LoopClosing::NewDetectCommonRegions() end 1" << endl;
        return false;
    }

    //cout << "LoopClosing::NewDetectCommonRegions() 02" << endl;
    if(mpTracker->mSensor == System::STEREO && mpLastMap->GetAllKeyFrames().size() < 5) //12
    {
        mpKeyFrameDB->add(mpCurrentKF);
        mpCurrentKF->SetErase();
	//cout << "LoopClosing::NewDetectCommonRegions() end 2" << endl;
        return false;
    }

    //cout << "LoopClosing::NewDetectCommonRegions() 03" << endl;
    if(mpLastMap->GetAllKeyFrames().size() < 12)
    {
        mpKeyFrameDB->add(mpCurrentKF);
        mpCurrentKF->SetErase();
	//cout << "LoopClosing::NewDetectCommonRegions() end 3" << endl;
        return false;
    }
    ...
    //cout << "LoopClosing::NewDetectCommonRegions() 04" << endl;
    if(mnLoopNumCoincidences > 0)
    {
        ...
        //cout << "LoopClosing::NewDetectCommonRegions() 05" << endl;
        if(bCommonRegion)
        {
    ...
    //cout << "LoopClosing::NewDetectCommonRegions() 06" << endl;
    //Merge candidates
    ...
    //cout << "LoopClosing::NewDetectCommonRegions() 07" << endl;
    if(mbMergeDetected || mbLoopDetected)
    {
        //f_time_pr << "Geo" << " " << timeGeoKF_ms.count() << endl;
        mpKeyFrameDB->add(mpCurrentKF);
	//cout << "LoopClosing::NewDetectCommonRegions() end 4 true" << endl;
        return true;
    }
...
/*#ifdef COMPILEDWITHC11
    ...
    //cout << "LoopClosing::NewDetectCommonRegions() 08" << endl;
    if(!bMergeDetectedInKF && !vpMergeBowCand.empty())
    {
	cout << "LoopClosing::NewDetectCommonRegions() 08.0" << endl;
        mbMergeDetected = DetectCommonRegionsFromBoW(vpMergeBowCand, mpMergeMatchedKF, mpMergeLastCurrentKF, mg2oMergeSlw, mnMergeNumCoincidences, mvpMergeMPs, mvpMergeMatchedMPs);
	cout << "LoopClosing::NewDetectCommonRegions() 08.1" << endl;
    }

    //cout << "LC: add to KFDB" << endl;
    cout << "LoopClosing::NewDetectCommonRegions() 09" << endl;
    mpKeyFrameDB->add(mpCurrentKF);
    cout << "LoopClosing::NewDetectCommonRegions() 10" << endl;

    if(mbMergeDetected || mbLoopDetected)
    {
	//cout << "LoopClosing::NewDetectCommonRegions() end 5 true" << endl;
        return true;
    }
    ...
    //cout << "LoopClosing::NewDetectCommonRegions() end 6" << endl;
    return false;
}
...
{
    cout << "LoopClosing::DetectCommonRegionsFromBoW start" << endl;
    ...
    cout << "LoopClosing::DetectCommonRegionsFromBoW 01" << endl;
    ...
    cout << "LoopClosing::DetectCommonRegionsFromBoW 02" << endl;
    int index = 0;
	...
	cout << "LoopClosing::DetectCommonRegionsFromBoW 02.1" << endl;
	...
	cout << "LoopClosing::DetectCommonRegionsFromBoW 02.2" << endl;
        int nIndexMostBoWMatchesKF=0;
        for(int j=0; j<vpCovKFi.size(); ++j)
        {
            if(!vpCovKFi[j] || vpCovKFi[j]->isBad())
                continue;
	    cout << "LoopClosing::DetectCommonRegionsFromBoW 02.2.1" << endl;
            int num = matcherBoW.SearchByBoW(mpCurrentKF, vpCovKFi[j], vvpMatchedMPs[j]);
	    cout << "LoopClosing::DetectCommonRegionsFromBoW 02.2.2" << endl;
	...
	cout << "LoopClosing::DetectCommonRegionsFromBoW 02.3" << endl;
	...
	cout << "LoopClosing::DetectCommonRegionsFromBoW 02.4" << endl;
	    ...
	    cout << "LoopClosing::DetectCommonRegionsFromBoW 02.5" << endl;
            bool bFixedScale = mbFixScale;
            if(mpTracker->mSensor==System::IMU_MONOCULAR && !mpCurrentKF->GetMap()->GetIniertialBA2())
                bFixedScale=false;

	    cout << "LoopClosing::DetectCommonRegionsFromBoW 02.5.1" << endl;
	    cout << "pMostBoWMatchesKF is NULL? " << (pMostBoWMatchesKF->mpCamera == NULL) << " mpCurrentKF is NULL? : " << (mpCurrentKF->mpCamera == NULL) << endl;
            Sim3Solver solver = Sim3Solver(mpCurrentKF, pMostBoWMatchesKF, vpMatchedPoints, bFixedScale, vpKeyFrameMatchedMP);
	    cout << "LoopClosing::DetectCommonRegionsFromBoW 02.5.2" << endl;
            solver.SetRansacParameters(0.99, nBoWInliers, 300); // at least 15 inliers

	    cout << "LoopClosing::DetectCommonRegionsFromBoW 02.5.3" << endl;
	    ...
	    cout << "LoopClosing::DetectCommonRegionsFromBoW 02.6" << endl;
            while(!bConverge && !bNoMore)
            {
                mTcm = solver.iterate(20,bNoMore, vbInliers, nInliers, bConverge);
            }

	    cout << "LoopClosing::DetectCommonRegionsFromBoW 02.7" << endl;
    ...
    cout << "LoopClosing::DetectCommonRegionsFromBoW 03" << endl;
	...
	cout << "LoopClosing::DetectCommonRegionsFromBoW end 1" << endl;
...
//        f_succes_pr << "% NumCand: " << std::to_string(numCandidates) << "; matches: " << std::to_string(maxMatched) << endl;
    ...
    cout << "LoopClosing::DetectCommonRegionsFromBoW end 2" << endl;
    return false;
}
...
void LoopClosing::CorrectLoop()
{
    ...
    file_out.open("timeseris.txt", std::ios_base::app);
    gettimeofday(&start, NULL);
    cout << "LoopClosing::CorrectLoop Loop detected!" << endl;
    ...
    gettimeofday(&end, NULL);
    double checkCorrectLoop = (end.tv_sec * 1000000 + end.tv_usec)  - (start.tv_sec * 1000000 + start.tv_usec );
    file_out << "-------checkCorrectLoop  mpLocalMapper->isStopped() " << checkCorrectLoop<< endl;
...

    {
        ...
        for(vector<KeyFrame*>::iterator vit=mvpCurrentConnectedKFs.begin(), vend=mvpCurrentConnectedKFs.end(); vit!=vend; vit++)
        {
            ...
            counter++;
            gettimeofday(&end, NULL);
            double checkCorrectLoop1 = (end.tv_sec * 1000000 + end.tv_usec)  - (start.tv_sec * 1000000 + start.tv_usec );
            file_out << "-------------- in CorrectLoop() mvpCurrentConnectedKFs counter: " << counter << ";   ---- time is: " << checkCorrectLoop1<< endl;
        ...
        for(KeyFrameAndPose::iterator mit=CorrectedSim3.begin(), mend=CorrectedSim3.end(); mit!=mend; mit++)
        {
            ...
            counter++;
            gettimeofday(&end, NULL);
            double checkCorrectLoop2 = (end.tv_sec * 1000000 + end.tv_usec)  - (start.tv_sec * 1000000 + start.tv_usec );
            file_out << "-------------- in CorrectLoop()2 CorrectedSim3 counter: " << counter << ";   ---- time is: " << checkCorrectLoop2<< endl;
   ...
   //     cout << "LC: end correcting KeyFrames" << endl;
        ...
        for(size_t i=0; i<mvpLoopMatchedMPs.size(); i++)
        {
            ...
            counter++;
            gettimeofday(&end, NULL);
            double checkCorrectLoop3 = (end.tv_sec * 1000000 + end.tv_usec)  - (start.tv_sec * 1000000 + start.tv_usec );
            file_out << "-------------- in CorrectLoop()3 mvpLoopMatchedMPs counter: " << counter << ";   ---- time is: " << checkCorrectLoop3<< endl;
    ...
    gettimeofday(&end, NULL);
    double checkCorrectLoop4 = (end.tv_sec * 1000000 + end.tv_usec)  - (start.tv_sec * 1000000 + start.tv_usec );
    file_out << "-------------- in CorrectLoop()3 end replacing duplicated ---- time is: " << checkCorrectLoop4<< endl;
    ...
    SearchAndFuse(CorrectedSim3, mvpLoopMapPoints);//06/21/2021
    //cout << "LC: end SearchAndFuse" << endl;

 gettimeofday(&end, NULL);
    double checkCorrectLoop5 = (end.tv_sec * 1000000 + end.tv_usec)  - (start.tv_sec * 1000000 + start.tv_usec );
    file_out << "-------------- SearchAndFuse finshed time is: " << checkCorrectLoop5<< endl;
    ...
    for(vector<KeyFrame*>::iterator vit=mvpCurrentConnectedKFs.begin(), vend=mvpCurrentConnectedKFs.end(); vit!=vend; vit++)
    {
        ...
        for(vector<KeyFrame*>::iterator vit2=mvpCurrentConnectedKFs.begin(), vend2=mvpCurrentConnectedKFs.end(); vit2!=vend2; vit2++)
        {
            ...
            counter++;
            gettimeofday(&end, NULL);
            double checkCorrectLoop6 = (end.tv_sec * 1000000 + end.tv_usec)  - (start.tv_sec * 1000000 + start.tv_usec );
            file_out << "-------------- in CorrectLoop()4 mvpCurrentConnectedKFs counter: " << counter << ";   ---- time is: " << checkCorrectLoop6<< endl;


    }
 //   cout << "LoopClosing::CorrectLoop LC: end updating covisibility graph" << endl;
    ...
    cout << "LoopClosing::CorrectLoop Optimize essential graph" << endl;
    if(pLoopMap->IsInertial() && pLoopMap->isImuInitialized())
    {
        //cout << "With 4DoF" << endl;
	//xukan should uncomment
    ...
    gettimeofday(&end, NULL);
    double checkCorrectLoop7 = (end.tv_sec * 1000000 + end.tv_usec)  - (start.tv_sec * 1000000 + start.tv_usec );
    file_out << "-------------- in CorrectLoop()5 Optimize;   ---- time is: " << checkCorrectLoop7<< endl;



    cout << "LoopClosing::CorrectLoop Optimize essential graph finished" << endl;
    //usleep(5*1000*1000);
    ...
    gettimeofday(&end, NULL);
    double checkCorrectLoop8 = (end.tv_sec * 1000000 + end.tv_usec)  - (start.tv_sec * 1000000 + start.tv_usec );
    file_out << "-------------- in CorrectLoop()6 inform big change ;   ---- time is: " << checkCorrectLoop8<< endl;
    ...
    if(!pLoopMap->isImuInitialized() || (pLoopMap->KeyFramesInMap()<200 && mpAtlas->CountMaps()==1))
    {
        ...
        std::cout<< "globle bbundle adjustment"<< endl;
        mbRunningGBA = true;
        mbFinishedGBA = false;
        mbStopGBA = false;


        cout << "RunGlobalBundleAdjustment 1" << endl;
        mpThreadGBA = new thread(&LoopClosing::RunGlobalBundleAdjustment, this, pLoopMap, mpCurrentKF->mnId);
        cout << "the thread is finished then release localmapper" << endl;
    }

    // Loop closed. Release Local Mapping.
    mpLocalMapper->Release();

    cout << "mpLocalMapper Released" << endl;
...
void LoopClosing::MergeLocal()
{
  //  cout << "LoopClosing::MergeLocal() 00" << endl;
   ...
   /// cout << "LoopClosing::MergeLocal() 01" << endl;
    ...
    if(isRunningGBA())
    {
        ...
        {
            mpThreadGBA->detach();
            std::cout <<"aborting Global Bundle Adjustment" << std::endl;
            delete mpThreadGBA;
        }
        bRelaunchBA = true;
    }
   // cout << "LoopClosing::MergeLocal() 02" << endl;
   ...
   // cout << "LoopClosing::MergeLocal() 03" << endl;
  ...
  //  cout << "LoopClosing::MergeLocal() 04" << endl;
   ...
   // cout << "LoopClosing::MergeLocal() 05" << endl;
  ...
  //  cout << "LoopClosing::MergeLocal() 05.x.1" << endl;
#ifdef COMPILEDWITHC11
   ...
   // cout << "LoopClosing::MergeLocal() 05.x.2" << endl;
    //TODO Time test
#ifdef COMPILEDWITHC11
...
#endif
    {
       ...
       // cout << "LoopClosing::MergeLocal() 05.x.3" << endl;
        for(KeyFrame* pKFi : spLocalWindowKFs)
        {
            if(!pKFi || pKFi->isBad())
            {
	   ...
	   // cout << "LoopClosing::MergeLocal() 05.x.3.1" << endl;
            // Make sure connections are updated
            pKFi->UpdateMap(pMergeMap);
	   // cout << "LoopClosing::MergeLocal() 05.x.3.2" << endl;
            pKFi->mnMergeCorrectedForKF = mpCurrentKF->mnId;
	  //  cout << "LoopClosing::MergeLocal() 05.x.3.3" << endl;
	    if(pMergeMap == NULL){
		cout << "--------pMergeMap == NULL--------" << endl;
 	    }
            pMergeMap->AddKeyFrame(pKFi);
	 //   cout << "LoopClosing::MergeLocal() 05.x.3.4" << endl;
            pCurrentMap->EraseKeyFrame(pKFi);

	//    cout << "LoopClosing::MergeLocal() 05.x.3.5" << endl;
            if(pCurrentMap->isImuInitialized())
            {
                pKFi->SetVelocity(pKFi->mVwbMerge);
            }
        }
//	cout << "LoopClosing::MergeLocal() 05.x.6" << endl;
        for(MapPoint* pMPi : spLocalWindowMPs)
        {
            if(!pMPi || pMPi->isBad())
                continue;
  ...
  //  cout << "LoopClosing::MergeLocal() 05.x.7" << endl;
#ifdef COMPILEDWITHC11
  ...
  //  cout << "LoopClosing::MergeLocal() 05.x" << endl;
    ...
    // into the current keyframe and neighbors using corrected poses.
    // Fuse duplications.
  //  cout << "LoopClosing::MergeLocal() 05.1" << endl;
    SearchAndFuse(vCorrectedSim3, vpCheckFuseMapPoint);
  //  cout << "LoopClosing::MergeLocal() 05.2" << endl;
   ...
   // cout << "LoopClosing::MergeLocal() 06" << endl;
    // Update connectivity
   ...
   // cout << "LoopClosing::MergeLocal() 07" << endl;
    ...
    else
    {
        ...
        cout << "Now in Optimizer::LocalBundleAdjustment" << endl;
   ...
   // cout << "LoopClosing::MergeLocal() 08" << endl;
    //return;
    ...
    else
    {
        ...
        //Apply the transformation
        {
		   ...
		   // cout << "LoopClosing::MergeLocal() 08.1" << endl;
                    KeyFrame* pKFref = pMPi->GetReferenceKeyFrame();
                    g2o::Sim3 g2oCorrectedSwi = vCorrectedSim3[pKFref].inverse();
                    g2o::Sim3 g2oNonCorrectedSiw = vNonCorrectedSim3[pKFref];
                    ...
                    Eigen::Matrix<double,3,1> eigP3Dw = Converter::toVector3d(P3Dw);
                    Eigen::Matrix<double,3,1> eigCorrectedP3Dw = g2oCorrectedSwi.map(g2oNonCorrectedSiw.map(eigP3Dw));
		    //cout << "LoopClosing::MergeLocal() 08.2" << endl;
                    cv::Mat cvCorrectedP3Dw = Converter::toCvMat(eigCorrectedP3Dw);
                    pMPi->SetWorldPos(cvCorrectedP3Dw);

                    pMPi->UpdateNormalAndDepth();
		   // cout << "LoopClosing::MergeLocal() 08.3" << endl;
                }
            }
        }
        Verbose::PrintMess("MERGE-VISUAL: Apply transformation to all elements of the old map", Verbose::VERBOSITY_DEBUG);

	//cout << "LoopClosing::MergeLocal() 09" << endl;
    ...
    //cout << "LoopClosing::MergeLocal() 10" << endl;
    mpLocalMapper->Release();
...
struct timeval start, end;
    ...
    if(bRelaunchBA && (!pCurrentMap->isImuInitialized() || (pCurrentMap->KeyFramesInMap()<200 && mpAtlas->CountMaps()==1)))
    {
        ...
        mbFinishedGBA = false;
        mbStopGBA = false;
        std::cout<< "globle bbundle adjustment 2"<< std::endl;
        mpThreadGBA = new thread(&LoopClosing::RunGlobalBundleAdjustment,this, pMergeMap, mpCurrentKF->mnId);
    }

    gettimeofday(&end, NULL);
             double check6 = (end.tv_sec * 1000000 + end.tv_usec)  - (start.tv_sec * 1000000 + start.tv_usec );
    file_out << "-------check6  new thread( " << check6<< endl;
...
mergeLocalFinished = true;
    //cout << "LoopClosing::MergeLocal() end" << endl;
...
void LoopClosing::SearchAndFuse(const KeyFrameAndPose &CorrectedPosesMap, vector<MapPoint*> &vpMapPoints)
{
    ...
    {
	cout << "LoopClosing::SearchAndFuse start" << endl;
        int num_replaces = 0;
	cout << "LoopClosing::SearchAndFuse 00.1" << endl;
        KeyFrame* pKFi = mit->first;
	if(pKFi == NULL) continue;
	cout << "LoopClosing::SearchAndFuse 00.2 pKFi IS null? " << (pKFi == NULL) << endl;
        Map* pMap = pKFi->GetMap();

	cout << "LoopClosing::SearchAndFuse 01" << endl;

        g2o::Sim3 g2oScw = mit->second;
        cv::Mat cvScw = Converter::toCvMat(g2oScw);

	cout << "LoopClosing::SearchAndFuse 02" << endl;
	...
	cout << "LoopClosing::SearchAndFuse 03" << endl;
	...
	cout << "LoopClosing::SearchAndFuse 04" << endl;
...
{
    cout << "LoopClosing::SearchAndFuse 00" << endl;
    ORBmatcher matcher(0.8);
    ...
    cout << "LoopClosing::SearchAndFuse end" << endl;
}
...
void LoopClosing::RunGlobalBundleAdjustment(Map* pActiveMap, unsigned long nLoopKF)
{
    ...
    cout << "checking imu: 0" <<  endl;

    Verbose::PrintMess("Starting Global Bundle Adjustment", Verbose::VERBOSITY_NORMAL);

cout << "checking imu: 1" <<  endl;
    const bool bImuInit = pActiveMap->isImuInitialized();
    cout << "checking imu: " << bImuInit<<  endl;
    cout << "Before Optimazer !mbStopGBA  " << !mbStopGBA << endl;

    if(!bImuInit){
        Optimizer::GlobalBundleAdjustemnt(pActiveMap,10,&mbStopGBA,nLoopKF,false);
        cout << "Optimizer::GlobalBundleAdjustemnt " <<  endl;
    }
    else{
        Optimizer::FullInertialBA(pActiveMap,7,false,nLoopKF,&mbStopGBA);
        cout << "Optimizer::FullInertialBA(" <<  endl;
    }
    ...
    {
    cout << " before chekcing,  idx!=mnFullBAIdx " << (idx!=mnFullBAIdx) << "!bImuInit " << !bImuInit << "pActiveMap->isImuInitialized()"<< pActiveMap->isImuInitialized() << endl;
        ...
        cout << "idx!=mnFullBAIdx " << (idx!=mnFullBAIdx) << "!bImuInit " << !bImuInit << "pActiveMap->isImuInitialized()"<< pActiveMap->isImuInitialized() << endl;


         cout << "After Optimazer !mbStopGBA  " << !mbStopGBA << endl;
        if(!mbStopGBA)
        {
            ...
            mpLocalMapper->RequestStop();
            // Wait until Local Mapping has effectively stopped
            cout << " mpLocalMapper Stopped " <<  endl;
            ...
            cout << "!mpLocalMapper->isStopped() && !mpLocalMapper->isFinished()" << (!mpLocalMapper->isStopped() && !mpLocalMapper->isFinished()) << endl;


            cout << "RunGlobalBundleAdjustment chekcing 1 " <<  endl;
            while(!lpKFtoCheck.empty())
            {
                ...
                cout << "RunGlobalBundleAdjustment chekcing 2 " <<  endl;
                ...
                cout << "RunGlobalBundleAdjustment chekcing3  " <<  endl;
    ...
    //TODO __ YUnshu
                 if(pKF->bImu)
                 {
                //     cout << "Set New Bias started" << endl;
                //     //cout << "-------Update inertial values" << endl;
                     pKF->mVwbBefGBA = pKF->GetVelocity();
                //     cout << "Set New Bias is 1" << endl;
                //     if (pKF->mVwbGBA.empty())
                //         Verbose::PrintMess("pKF->mVwbGBA is empty", Verbose::VERBOSITY_NORMAL);


                //     cout << "Set New Bias is 2" << endl;

                //     assert(!pKF->mVwbGBA.empty());
                //     cout << "Set New Bias is 3" << endl;

                     pKF->SetVelocity(pKF->mVwbGBA);
                //     cout << "Set New Bias is 4" << endl;


                //     cout << "Here is the problem: we don't have mBiasGBA" << endl;
                     pKF->SetNewBias(pKF->mBiasGBA);

                //     cout << "Set New Bias is done" << endl;
                 }
            ...
            cout << "RunGlobalBundleAdjustment chekcing 4 " <<  endl;

            for(size_t i=0; i<vpMPs.size(); i++)
            {
                cout << "RunGlobalBundleAdjustment chekcing 4.1 " <<  endl;

                MapPoint* pMP = vpMPs[i];

                cout << "RunGlobalBundleAdjustment chekcing 4.2 " <<  endl;
                ...
                {
                    cout << "RunGlobalBundleAdjustment chekcing 4.is bad " <<  endl;
                    // If optimized by Global BA, just update
                    pMP->SetWorldPos(pMP->mPosGBA);
                }
                else
                {
                    cout << "RunGlobalBundleAdjustment chekcing 4.is good " <<  endl;
                    // Update according to the correction of its reference keyframe
                    KeyFrame* pRefKF = pMP->GetReferenceKeyFrame();
        if (pRefKF == NULL)
             continue;

cout << "RunGlobalBundleAdjustment chekcing 4.5 " <<  endl;
cout << "So ,it's the  pRefKF->mnBAGlobalForK problem??" <<  endl;

cout << "pRefKF->mnBAGlobalForKF  " <<  pRefKF->mnBAGlobalForKF << endl;
cout << "nLoopKF  " <<  nLoopKF << endl;
                    if(pRefKF->mnBAGlobalForKF!=nLoopKF)
                        continue;

                        cout << "RunGlobalBundleAdjustment chekcing 4.55 " <<  endl;


                    if(pRefKF->mTcwBefGBA.empty())
                        continue;

                        cout << "RunGlobalBundleAdjustment chekcing 4.6 " <<  endl;
                    ...
                    cout << "RunGlobalBundleAdjustment chekcing 4.9 " <<  endl;

                }
            }
            cout << "RunGlobalBundleAdjustment chekcing 5 " <<  endl;

            pActiveMap->InformNewBigChange();
            pActiveMap->IncreaseChangeIndex();

            // TODO Check this update
            //y mpTracker->UpdateFrameIMU(1.0f, mpTracker->GetLastKeyFrame()->GetImuBias(), mpTracker->GetLastKeyFrame());
    ...
    cout << "the thread is finished" << endl;

    gettimeofday(&end, NULL);
             double check7 = (end.tv_sec * 1000000 + end.tv_usec)  - (start.tv_sec * 1000000 + start.tv_usec );
    file_out << "-------check end  globalBundleAdjustment thread" << check7<< endl;
