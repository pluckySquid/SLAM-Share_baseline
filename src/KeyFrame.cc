...
long unsigned int KeyFrame::nNextId_backup = 0;
long unsigned int KeyFrame::nNextId_max = 0;
bool KeyFrame::needIndexProcess = false;
...
{
    //cout << "KeyFrame::UpdateBestCovisibles() start" << endl;
    unique_lock<mutex> lock(mMutexConnections);
    ...
    //cout << "KeyFrame::UpdateBestCovisibles() 01" << endl;
    sort(vPairs.begin(),vPairs.end());
    list<KeyFrame*> lKFs;
    list<int> lWs;
    //cout << "KeyFrame::UpdateBestCovisibles() 02, vPairs.size(): " << vPairs.size() << endl;
    for(size_t i=0, iend=vPairs.size(); i<iend;i++)
    {
	//cout << "KeyFrame::UpdateBestCovisibles() 02.1" << endl;
        if(!vPairs[i].second->isBad())//problem
        {
	    //cout << "KeyFrame::UpdateBestCovisibles() 02.2" << endl;
            lKFs.push_front(vPairs[i].second);
	    //cout << "KeyFrame::UpdateBestCovisibles() 02.3" << endl;
            lWs.push_front(vPairs[i].first);
        }
	//cout << "KeyFrame::UpdateBestCovisibles() 02.4" << endl;
    }
    //cout << "KeyFrame::UpdateBestCovisibles() 03" << endl;
    mvpOrderedConnectedKeyFrames = vector<KeyFrame*>(lKFs.begin(),lKFs.end());
    mvOrderedWeights = vector<int>(lWs.begin(), lWs.end());
    //cout << "KeyFrame::UpdateBestCovisibles() end" << endl;
}
...
void KeyFrame::PostLoad(map<long unsigned int, KeyFrame*>& mpKFid, map<long unsigned int, MapPoint*>& mpMPid, map<unsigned int, GeometricCamera*>& mpCamId){
    ...
    cout << "KeyFrame::PostLoad 00" << endl;
    // Pose
    SetPose(Tcw);
    ...
    cout << "KeyFrame::PostLoad 01" << endl;
    ...
    cout << "KeyFrame::PostLoad 02" << endl;
    ...
    cout << "KeyFrame::PostLoad 03" << endl;
    // Loop edge KeyFrame
    mspLoopEdges.clear();
    ...
    cout << "KeyFrame::PostLoad 04" << endl;
    //Camera data
    ...
    cout << "KeyFrame::PostLoad 05" << endl;
    //Inertial data
    ...
    cout << "KeyFrame::PostLoad 06" << endl;
    ...
    //cout << "KeyFrame::PostLoad 07" << endl;
    //if(needIndexProcess){
	////update index;
	//mnId = nNextId++;
...
void KeyFrame::PostLoad_second(map<long unsigned int, KeyFrame*>& mpKFid, map<long unsigned int, MapPoint*>& mpMPid, map<unsigned int, GeometricCamera*>& mpCamId){
    // Rebuild the empty variables
    //cout << "KeyFrame::PostLoad 00" << endl;
    // Pose
    SetPose(Tcw);
    ...
    //assert(mpCamera == NULL);
    if(mnBackupIdCamera2 >= 0)
    {
        mpCamera2 = mpCamId[mnBackupIdCamera2];
    }
    //assert(mpCamera2 != NULL);
    ...
    //if(needIndexProcess){
	//update index;
	//+ an offset to avoid confliction
	//mnId = mnId + nNextId_backup;
	//needIndexProcess = false;
   // }
}
