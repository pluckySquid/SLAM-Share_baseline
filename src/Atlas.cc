...
Atlas::Atlas(){
    ...
    std:: cout << "after null construction " << std::endl;
}
...
Atlas::Atlas(int initKFid): mnLastInitKFidMap(initKFid), mHasViewer(false)
{
        ...
        std:: cout << "atlas is it here?? " << std::endl;
...
Atlas::~Atlas()
{
    std:: cout << "IN atlas construction " << std::endl;
...
void Atlas::AddKeyFrame(KeyFrame* pKF, bool skip)
{
   // cout << "map adding to: " << pKF->GetMap() << endl;
    Map* pMapKF = pKF->GetMap();
    pMapKF->AddKeyFrame(pKF, skip);
}

void Atlas::AddMapPoint(MapPoint* pMP)
{
    //cout << "mappoints adding to: " << pMP->GetMap() << endl;
...
{
    //cout << "Atlas::GetAllMaps() mspMaps size: " << mspMaps.size() << endl;
    unique_lock<mutex> lock(mMutexAtlas);
    struct compFunctor
    {
...
void Atlas::PreSave()
{
    ...
    std::cout << "we are close to find the error" << std::endl;
    ...
    for(Map* pMi : mvpBackupMaps)
    {
        ...
        if(!pMi || pMi->IsBad()){
         ...
         std::cout<<"pMi->IsBad() " <<std::endl;
            continue;
        }

        if(pMi->GetAllKeyFrames().size() == 0) {
            std::cout<<"pMi->GetAllKeyFrames().size() == 0 " <<std::endl;
    ...
    for(GeometricCamera* pCam : mvpCameras)
    {
	    ...
	    std::cout << "saving, Using Pinhole" << std::endl;
        }
        else if(pCam->GetType() == pCam->CAM_FISHEYE)
        {
            mvpBackupCamKan.push_back((KannalaBrandt8*) pCam);
	    std::cout << "saving, Using KannalaBrandt8" << std::endl;
        }
    }
...
void Atlas::PostLoad_second(){
    ...
    unsigned long int numKF = 0, numMP = 0;
    map<long unsigned int, KeyFrame*> mpAllKeyFrameId;
    //cout << "Atlas::PostLoad_second(): " << mvpBackupMaps.size() << endl;
    for(Map* pMi : mvpBackupMaps)
    {
        //cout << "Map id:" << pMi->GetId() << endl;
        mspMaps.insert(pMi);
        map<long unsigned int, KeyFrame*> mpKeyFrameId;
        pMi->PostLoad_second(mpKeyFrameDB, mpORBVocabulary, mpKeyFrameId, mpCams);
        //mpAllKeyFrameId.insert(mpKeyFrameId.begin(), mpKeyFrameId.end());

        cout<< "before insertion: numKF = " << numKF << "; pMi->GetAllKeyFrames().size() = " << pMi->GetAllKeyFrames().size() << endl;
        numKF += pMi->GetAllKeyFrames().size();
        numMP += pMi->GetAllMapPoints().size();
