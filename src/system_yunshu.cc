...
/*
    ...
    struct timeval start0, start1, start2, start3, start4, start5, end0;
    ...
    tmp_second = "MH03_mono_0706.osa";
    //string tmp_second = "MH02_mono_0707.osa";
    if(!tmp_second.empty()){//second map
        bool isRead = LoadAtlas_second(tmp_second,BINARY_FILE);
    	cout << "***** second load starts*****, strLoadingFile_second: " << tmp_second << endl;
    	if(isRead){
    ...
    if(!tmp_second.empty()){//second map
        ...
        if(isRead){
            ...
            cout << "load_second 03" << endl;
            mpAtlas->SetReferenceMapPoints(m->GetReferenceMapPoints());
            cout << "load_second 04, vm size:" << vm.size() << endl;
            }
            //getline(cin,tmp);
            cout << "*******FINISHED FIRST STEP TO MERGE TWO MAPS*******" << endl;
            for(int i = 0; i < vm.size(); i++){
	...
	//cout << "------press enter to continue------ END of PROGRAM" << endl;
	//string tmp;

    gettimeofday(&end0, NULL);
    double elapsed1 = ((start2.tv_sec - start1.tv_sec) * 1000) + (start2.tv_usec / 1000 - start1.tv_usec / 1000);
    double elapsed2 = ((start3.tv_sec - start2.tv_sec) * 1000) + (start3.tv_usec / 1000 - start2.tv_usec / 1000);
    double elapsed3 = ((start4.tv_sec - start3.tv_sec) * 1000) + (start4.tv_usec / 1000 - start3.tv_usec / 1000);
    double elapsed4 = ((start5.tv_sec - start4.tv_sec) * 1000) + (start5.tv_usec / 1000 - start4.tv_usec / 1000);
    double elapsed5 = ((end0.tv_sec - start5.tv_sec) * 1000) + (end0.tv_usec / 1000 - start5.tv_usec / 1000);
    double elapsed = ((end0.tv_sec - start1.tv_sec) * 1000) + (end0.tv_usec / 1000 - start1.tv_usec / 1000);
    cout << "elapsed time 1 " << elapsed1 << endl;
    cout << "elapsed time 2 " << elapsed2 << endl;
    cout << "elapsed time 3 " << elapsed3 << endl;
    cout << "elapsed time 4 " << elapsed4 << endl;
    cout << "elapsed time 5 " << elapsed5 << endl;
	cout << "elapsed time all " << elapsed << endl;
*/
    //getline(cin,tmp);
}

void System::setisRead_Yunshu(bool isRead){
 //isRead = isread;
...
void System::ChangeTempMap_Yunshu(bool isRead){
...
void System::SaveAtlas(int type){
	...
	std::cout << "mpKeyFrameDatabase PreSave() end" << std::endl;
        ...
        double elapsed = ((presaveEnd0.tv_sec - presaveStart0.tv_sec) * 1000) + (presaveEnd0.tv_usec / 1000 - presaveStart0.tv_usec / 1000);
