...
bool Map::needIndexProcess = false;
...
int Map::GetKeyFrameSize(){
    return (int)mspKeyFrames.size();
}

void Map::AddKeyFrame(KeyFrame *pKF, bool skip)
{
    if(pKF == NULL){
	std::cout << "ap::AddKeyFrame 00 pKF == NULL" << std::endl;
    }
    //std::cout << "Map::AddKeyFrame 01" << std::endl;
    unique_lock<mutex> lock(mMutexMap);
    //std::cout << "Map::AddKeyFrame 02" << std::endl;
    ...
    //std::cout << "Map::AddKeyFrame 03" << std::endl;
    mspKeyFrames.insert(pKF);
    //std::cout << "Map::AddKeyFrame 05" << std::endl;
    if(pKF->mnId>mnMaxKFid)
    {
        mnMaxKFid=pKF->mnId;
    }
    //std::cout << "Map::AddKeyFrame 06" << std::endl;
    if(!skip && pKF->mnId<mpKFlowerID->mnId)
    {
	//std::cout << "Map::AddKeyFrame 07" << std::endl;
        mpKFlowerID = pKF;
	//std::cout << "Map::AddKeyFrame 08" << std::endl;
    }
   // std::cout << "Map::AddKeyFrame end" << std::endl;
}
...
{
    if(mspMapPoints.size() == 0 || mspKeyFrames.size() == 0) return;
    int nMPWithoutObs = 0;
    ...
    cout << "Map::PreSave 00, size: " << mvpKeyFrameOrigins.size() << endl;
    for(int i = 0, numEl = mvpKeyFrameOrigins.size(); i < numEl; ++i)
    {
        mvBackupKeyFrameOriginsId.push_back(mvpKeyFrameOrigins[i]->mnId);
    }
    cout << "Map::PreSave 01" << endl;
    mvpBackupMapPoints.clear();
    cout << "Map::PreSave 02 mspMapPoints size: " << mspMapPoints.size() << endl;
...
void Map::PostLoad(KeyFrameDatabase* pKFDB, ORBVocabulary* pORBVoc, map<long unsigned int, KeyFrame*>& mpKeyFrameId, map<unsigned int, GeometricCamera*> &mpCams)
{
    ...
    for(MapPoint* pMPi : mspMapPoints)
    { if(!pMPi || pMPi->isBad()){
            cout << "This is a bad pMPi"<< endl;
            
            continue;

        }
         //cout << "This is a good pMPi"<< endl;
            
        //cout << "Post-Load of mappoint " << pMPi->mnId << endl;
        pMPi->PostLoad(mpKeyFrameId, mpMapPointId);
    }
    cout << "End to rebuild MapPoint references first" << endl;
    ...
    for(KeyFrame* pKFi : mspKeyFrames)
    {
        ...
        cout << "mspKeyFrames.size()" << mspKeyFrames.size()<< endl;
        if(!pKFi || pKFi->isBad()){
            cout << "Map This is a bad pKFi"<< endl;
            
            continue;

        }
       
                    cout << "Map This is a good pKFi"<< endl;
	...
	//xukan add
        ...
        cout << "Checking Xukan's add "<< endl;
        if(mnBackupKFlowerID != -1 && pKFi->mnId == mnBackupKFlowerID){
	    mpKFlowerID = pKFi;
	    cout << "Map::PostLoad mpKFlowerID = pKFi;" << endl;
        }
	if(mnBackupKFinitialID != -1 && pKFi->mnId == mnBackupKFinitialID){
	    mpKFinitial = pKFi;
	    cout << "Map::PostLoad mpKFinitial = pKFi;" << endl;
	}
        //xukan add end
    }
    ...
    mvpBackupMapPoints.clear();
    if(needIndexProcess){
	needIndexProcess = false;
	mnId = nNextId++;
    }
}

void Map::PostLoad_second(KeyFrameDatabase* pKFDB, ORBVocabulary* pORBVoc, map<long unsigned int, KeyFrame*>& mpKeyFrameId, map<unsigned int, GeometricCamera*> &mpCams)
{
    std::copy(mvpBackupMapPoints.begin(), mvpBackupMapPoints.end(), std::inserter(mspMapPoints, mspMapPoints.begin()));
    std::copy(mvpBackupKeyFrames.begin(), mvpBackupKeyFrames.end(), std::inserter(mspKeyFrames, mspKeyFrames.begin()));
    ...
    for(KeyFrame* pKFi : mspKeyFrames)
    {
        ...
        pKFi->mnOriginMapId = 1;

        mpKeyFrameId[pKFi->mnId] = pKFi;//is this needed?
    }
        // References reconstruction between different instances
    for(MapPoint* pMPi : mspMapPoints)
    {
    ...
    for(KeyFrame* pKFi : mspKeyFrames)
    {
	cout << "Map::PostLoad_second 02" << endl;
        pKFi->PostLoad_second(mpKeyFrameId, mpMapPointId, mpCams);
	cout << "Map::PostLoad_second 03" << endl;
        //pKFDB->add(pKFi);
	cout << "Map::PostLoad_second 04" << endl;
    ...
    cout << "end of Map::PostLoad_second" << endl;
    mvpBackupMapPoints.clear();
    //if(needIndexProcess){
	//needIndexProcess = false;
	//mnId = nNextId++;
    //}
        //update keyframe's mnId, default _second all need to do this processing.(which is right?)
    for(KeyFrame* pKFi : mspKeyFrames){
        pKFi->mnId = pKFi->mnId + pKFi->nNextId;
	pKFi->nNextId_backup = pKFi->nNextId_backup > pKFi->mnId ? pKFi->nNextId_backup : pKFi->mnId;
    }
    //KeyFrame::nNextId = pKFi->nNextId_backup + 1;
    for(MapPoint* pMPi : mspMapPoints){
	pMPi->mnId = pMPi->mnId + pMPi->nNextId;
	pMPi->nNextId_backup = pMPi->nNextId_backup > pMPi->mnId ? pMPi->nNextId_backup : pMPi->mnId;
    }
    KeyFrame::nNextId = KeyFrame::nNextId_backup + 1;
    MapPoint::nNextId = MapPoint::nNextId_backup + 1;
}
