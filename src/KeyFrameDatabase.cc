    ...
    mpVoc(&voc)
{
    ...
    std::cout<< "  *****KeyFrameDatabase construction  *******" << endl;
...
void KeyFrameDatabase::add(KeyFrame *pKF)
{
        ...
        std::cout<< "  *****KeyFrameDatabase add  *******" << endl;
    unique_lock<mutex> lock(mMutex);



    for(DBoW2::BowVector::const_iterator vit= pKF->mBowVec.begin(), vend=pKF->mBowVec.end(); vit!=vend; vit++){
        std::cout<< "  *****KeyFrameDatabase.cc om mvInvertedFile[[ctpr  *******" << endl;
...
std::cout<<" lKFs.size() "<<  lKFs.size()  <<std::endl;

        if(lKFs.begin() == lKFs.end()){
            std::cout<<"   %%%%%%%%%%%List does not have anything"<<std::endl;
        }



        for(list<KeyFrame*>::iterator lit=lKFs.begin(), lend= lKFs.end(); lit!=lend; lit++)   //this is the line causing the segmentation fault
        {
            std::cout<< "  ***** 2.5 *******" << endl;
