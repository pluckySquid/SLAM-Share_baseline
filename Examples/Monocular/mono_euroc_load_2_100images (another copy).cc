  ...
  //  ORB_SLAM3::System SLAM(argv[1],argv[2],ORB_SLAM3::System::MONOCULAR, true, 0, "", "./testmap.osa");
    ...
    for(int ni=0; ni<nImages[0]; ni++, proccIm++)
    {
            ...
            if(ni % 500 == 0 && ni> 0 )
            {
               ...
               //bool isRead = SLAM.MergeTwoMaps_Yunshu("MH05_0909.osa",1);
               //bool isRead = SLAM.LoadAtlas_second("MH05_0909.osa",1);
              // SLAM.setLoadMap_Yunshu(isRead);
              // bool isRead = SLAM.MergeTwoMaps_Yunshu("tempB" + std::to_string(ni / 100 - 1) + ".osa",1);
                 //isRead = SLAM.LoadAtlas_second("tempB" + std::to_string(ni / 100 - 1) + ".osa",1);

                cout << "*****************LoadAtlas is done, now set loadMap:  " << " ******************* " << endl;

               SLAM.MergeTwoMaps_Yunshu ("MH0405-full.osa",1);
