...
int main(int argc, char **argv)
{
    ...
    for(int ni=0; ni<nImages[0]; ni++, proccIm++)
    {
            ...
            if(ni % 100 == 0 && ni > 0)
            {
               ...
               // bool isRead = SLAM.LoadAtlas("temp.osa",1);

                cout << "*****************LoadAtlas is done, now set loadMap:  " << " ******************* " << endl;

              //  SLAM.setLoadMap_Yunshu (isRead);
             //   SLAM.ChangeTempMap_Yunshu(isRead);
                //ORB_SLAM3::System SLAM(argv[1],argv[2],ORB_SLAM3::System::MONOCULAR, true, 0, "temp.osa" );
           ...
           // cout << "Here8:  "  << " ******************* " << endl;
            ...
            cv::Mat im2;
            int proccIm2 = 0;
            for (int i = ni -99; i <= ni; i++){
                cout << "i:  " << i << " ******************* " << endl;


                proccIm2++;
                im = cv::imread(vstrImageFilenames[seq + 1][i],CV_LOAD_IMAGE_UNCHANGED);
                double tframe2 = vTimestampsCam[seq + 1][i];

                if(im2.empty())
                {
                    cerr << endl << "Failed to load image at: "
                         <<  vstrImageFilenames[seq + 1][i] << endl;
                    return 1;
                }
                ...
                // Pass the image to the SLAM system
                // cout << "tframe = " << tframe << endl;
                SLAM_2.TrackMonocular(im2,tframe2); // TODO change to monocular_inertial
                ...
                // Wait to load the next frame
                T=0;
                if(i<nImages[seq + 1]-1)
                    T = vTimestampsCam[seq][i+1]-tframe;
                else if(i>0)
                    T = tframe-vTimestampsCam[seq + 1][i-1];
