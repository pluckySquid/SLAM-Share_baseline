...
int main(int argc, char **argv)
{
    ...
    if (bFileName)
    {cout << "file string(argv[argc-1]): " << string(argv[argc-2]) << endl;
 ...
 cout<<file_name.find("_") <<file_name.find("_") <<endl;
if (file_name.find("_")!=std::string::npos){
    cout<< file_name.substr(file_name.find("_")+1, 3) <<endl;
    string s = file_name.substr(file_name.find("_")+1, 3);
    cout<< s <<endl;
    start = std::stoi(s) * 100;
    cout<< start <<endl;
}
else{
    start = 0;
...
if (file_name.find("_")){
        ...
        cv::Mat im;
        int proccIm = 0;
        for(int ni=start; ni<end; ni++, proccIm++)
        {
                        cout << "ni:  " << ni << " ******************* " << endl;
    ...
    if (bFileName)
    {
        ...
        const string kf_file =  "kf_smallmapResult" + saveFileName.substr(saveFileName.size() - 8) + ".txt";
        const string f_file =  "f_smallmapResult" + saveFileName.substr(saveFileName.size() - 8) + ".txt";
