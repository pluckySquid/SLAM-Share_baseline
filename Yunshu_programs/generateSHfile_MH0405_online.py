...
#        B = "python2 evaluation/evaluate_ate_scale_color_final_markBoth_Online.py evaluation/Ground_truth/EuRoC_left_cam/merged_GT_samples.txt kf_dataset-MH0405-"+date+"_" + str(i).zfill(3) +".txt --plot kf_dataset-MH0405-"+interval+"_" + str(i).zfill(3) +".pdf "+ str(i) +" Yunshu_Results/"+date+"/deletedResults2/missingFramesFile_MH04.txt " +str(i)+" Yunshu_Results/" + date+"/deletedResults2/missingFramesFile_MH05.txt"
        ...
        A = "echo " + resultSava_fileName.split(".sh")[0] + " | ./Examples/Monocular/mono_euroc ./Vocabulary/ORBvoc.txt ./Examples/Monocular/EuRoC.yaml \"$pathDatasetEuroc\"/MH04 ./Yunshu_Results/" + date +"/deletedResults2/MH04_" +str(i) + "_" +interval+".txt \"$pathDatasetEuroc\"/MH05 ./Yunshu_Results/" +date +"/deletedResults2/MH05_"+str(i+1)+"_"+str(interval) +".txt dataset-MH0405-"+date+"_"+str(i).zfill(3) + "\n"
        B = "python2 evaluation/evaluate_ate_scale_color_final_markBoth_Online.py evaluation/Ground_truth/EuRoC_left_cam/merged_GT_samples.txt kf_dataset-MH0405-"+date+"_" + str(i).zfill(3) +".txt --plot kf_dataset-MH0405-"+str(interval)+"_" + str(i).zfill(3) +".pdf "+ str(i) +" Yunshu_Results/"+date+"/deletedResults2/missingFramesFile_MH04.txt " +str(i+1)+" Yunshu_Results/" + date+"/deletedResults2/missingFramesFile_MH05.txt"
